...
const Subscription = ({ username }: { username: string }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        do {
          fetchedRepos = await FetchData(`${user.url}/subscriptions?page=${page}&per_page=${perPage}`);
  ...
  const totalPages = Math.ceil(repoData.length / perPage);
  const currentRepos = repoData.slice((currentPage - 1) * perPage, currentPage * perPage);
